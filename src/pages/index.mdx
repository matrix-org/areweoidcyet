...
This is relatively subjective, but we feel relatively confortable to rely on the system browser for multiple reasons:
...
For now, `m.login.password` isn't going away, so existing scripts won't break during migration.
