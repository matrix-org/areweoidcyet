...
| 🚧 Support for <code>urn:matrix:device:uia:*</code> scopes | Permissioning for UIA endpoints | REQUIRED | 🚧 | 🚧 | 🚧 | 🚧 || Handle device ID custom scope <code>urn:matrix:device:client:XXXXXXXX</code> | Session/device management | REQUIRED | ✅ | ✅Using dynamic-scopes feature | ❌ it is unclear if this is possible | ❌ it is unclear if this is possible |
